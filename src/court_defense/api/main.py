...
@app.get("/pick-file")
def pick_file():
    """Open native OS file picker for selecting a text file."""
    try:
        import tkinter as tk
        from tkinter import filedialog
        root = tk.Tk()
        ...
        path = filedialog.askopenfilename(
            title="Вибери файл з часовими мітками",
...
def validate_file(file_path: str):
    """Validate if a file exists and return its info."""
    try:
        p = Path(file_path)
        if not p.exists():
            return {"valid": False, "error": f"Файл не знайдено: {file_path}"}
        if not p.is_file():
            return {"valid": False, "error": f"Це не файл: {file_path}"}
        size_kb = p.stat().st_size / 1024
        return {"valid": True, "size_kb": round(size_kb, 1)}
    except Exception as e:
...
async def cut_audio(
    ...
    Args:
        folder_path: Шлях до папки справи
        phrases_file_path: ПОВНИЙ шлях до файлу зі списком фраз
        api_key: Anthropic API key (опціонально)
    """
    if api_key:
        os.environ["ANTHROPIC_API_KEY"] = api_key
    ...
    if not folder.exists() or not folder.is_dir():
        raise HTTPException(400, f"Папка не знайдена: {folder_path}")

    # Валідація файлу фраз
    phrases_path = Path(phrases_file_path)
    if not phrases_path.exists():
        raise HTTPException(
            400,
            f"Файл з фразами не знайдено: {phrases_file_path}"
        )

    if not phrases_path.is_file():
        raise HTTPException(
            400,
            f"Це не файл: {phrases_file_path}"
        )

    folder_name = folder.name
    tid = services.create_task(f"🎵 Нарізка: {folder_name}")
    services._upd(tid, stage="preparing", progress=2,
                  message=f"Папка: {folder_path} | Фрази: {phrases_path.name}")

    background_tasks.add_task(services.start_audio_cutting, tid, folder_path, str(phrases_path))
    return {
        "task_id": tid,
        "folder": folder_path,
